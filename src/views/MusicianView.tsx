...
export async function renderMusician(slug: string, locale: Locale) {
  const cachedGetMusician = unstable_cache(
    () => getMusician(slug, locale),
...
export async function musicianStaticParams() {
  try {
    const payload = await getPayload({ config: configPromise })
...
export async function musicianMetadata(slug: string, locale: Locale) {
  const musician = await getMusician(slug, locale)
  if (!musician) return {}
